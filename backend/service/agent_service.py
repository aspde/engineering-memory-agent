...
def get_agent(
    ...
    ``max_steps`` overrides the default ``MAX_AGENT_STEPS`` ReAct bound for
    the caller — automated scans (patrol) pass a per-type budget, interactive
    chat keeps the default.  ``None`` falls back to ``config.max_agent_steps``.
