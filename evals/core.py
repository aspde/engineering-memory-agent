...
Before this module, ``evals.runner`` / ``llm_runner`` / ``task_runner``
...
- :class:`EvalResult` — the generic result container used by the LLM and task
  runners (``evals.llm_runner.LlmEvalResult`` and
  ``evals.task_runner.TaskEvalResult`` are aliases of it).  The retrieval
...
- :func:`load_jsonl_items` — the labeled-set loader shared by the three
  ground-truth modules, which keep their data in ``evals/data/*.jsonl``.
"""
...
# see :func:`zero_judge_keys`.  Kept importable from ``evals.llm_runner``
...
class EvalResult:
    ...
    Shape shared by the LLM behaviour suites (``evals.llm_runner``) and
    the task-level suite (``evals.task_runner``); the retrieval runner
...
def zero_judge_keys(row: dict[str, Any]) -> None:
    ...
    summary keys unset instead (see ``evals.llm_runner.run_extraction``).
    """
...
def load_jsonl_items(path: str, cls: type[Any]) -> list[Any]:
    """Load labeled items from a JSONL file under ``evals/data``.

    ``path`` is a filename (e.g. ``"ground_truth.jsonl"``) resolved against
    ``evals/data``.  ``cls`` is the item type: a frozen dataclass
