...
``evals.core`` (``LlmEvalResult`` is an alias of ``core.EvalResult``).
...
from evals.llm_judge import judge_answer, judge_summary
from evals.llm_metrics import (
    ANSWER_METRIC_KEYS,
    E2E_METRIC_KEYS,
...
# the constant lives in ``evals.core``.

# The generic result container is shared with the task suite via
# ``evals.core``; only the name is suite-specific.
...
def _finish(
    ...
    """Roll up a suite's rows via the shared :func:`evals.core.finish`."""
...
def make_default_selector() -> ToolSelector:
    from evals.llm_executors import make_tool_selector

    return make_tool_selector()


def make_default_extractor() -> Extractor:
    from evals.llm_executors import make_extractor

    return make_extractor()


def make_default_answer_generator() -> AnswerGenerator:
    from evals.llm_executors import make_answer_generator
...
def make_default_e2e_runner(
    *, top_k: int = 5, retrieval_mode: str = "memory"
    ...
    from evals.llm_executors import make_e2e_runner
