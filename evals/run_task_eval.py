...
``python -m evals.e2e_seed --clear`` first so the facts each task
...
Examples:
    ...
    python -m evals.run_task_eval --sample 3 --judge deterministic

    # Full run with LLM judges and a Markdown report
    python -m evals.run_task_eval --report-md evals/reports/task-eval-report.md

    # Regression gate: fail (exit 2) if completion or groundedness drop
    python -m evals.run_task_eval \
...
from evals.task_ground_truth import load_task_items, validate_task_dataset
from evals.task_report import summarize, write_json, write_markdown
from evals.thresholds import check_thresholds, print_threshold_failures


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m evals.run_task_eval",
