...
Mirrors ``evals.llm_runner.run_e2e``: each task in the labeled set is
...
:class:`TaskEvalResult` (an alias of the shared ``evals.core.EvalResult``).
...
- The runner accepts *any* executor callable (``TaskRunner``), so unit tests
  ...
  ``evals.task_executors.make_task_runner``.
...
from evals.core import EvalResult, finish, zero_judge_keys
...
from evals.task_executors import TaskOutcome, make_task_runner
