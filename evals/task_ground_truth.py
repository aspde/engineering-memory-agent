...
The retrieval eval (``evals.ground_truth``) measures which memories a
search returns, and the LLM behavior eval (``evals.llm_ground_truth``)
...
``source_content`` strings (``evals.e2e_seed`` seeds exactly those into
...
The eval drive path is ``evals.run_task_eval`` (CLI) with the corpus
seeded by ``python -m evals.e2e_seed --clear`` first.
...
The tasks themselves live in ``evals/data/tasks.jsonl``; this module
...
from evals.core import load_jsonl_items
...
def validate_task_dataset(items: list[TaskItem] | None = None) -> list[str]:
    ...
    # time).
    from evals.llm_ground_truth import load_e2e_items
        ...
        for fact in it.required_facts:
            ...
            if not any(str(fact) in text for text in seed_texts):
                warnings.append(
                    ...
                    "source_content — run `python -m evals.e2e_seed` and "
