...
The eval report file (``evals/reports/llm-eval-report.json``) is overwritten
...
(``evals/reports/llm-eval-baseline.json``) and prints per-metric deltas —
...
Usage:
    ...
    python -m evals.experiments.compare_baseline

    # Explicit report path, larger tolerance for LLM-judged runs:
    python -m evals.experiments.compare_baseline --report path/to/report.json --tolerance 0.05
...
# ``parents[2]`` because this module lives one level deeper than the report
# modules it compares against: experiments/ → evals → repo root.
BASELINE_PATH = Path(__file__).resolve().parents[2] / "evals" / "reports" / "llm-eval-baseline.json"
...
def main() -> int:
    ...
    parser.add_argument(
        "--report",
        default="evals/reports/llm-eval-report.json",
        help="Fresh eval report JSON (default: evals/reports/llm-eval-report.json).",
    )
    parser.add_argument(
        "--baseline",
        default=str(BASELINE_PATH),
        help="Baseline JSON (default: evals/reports/llm-eval-baseline.json).",
    )
    parser.add_argument(
        "--tolerance",
        type=float,
    ...
    if report_judge and base_judge and report_judge != base_judge:
        print(
            ...
            "(e.g. --baseline evals/reports/llm-eval-semantic-baseline.json).",
            file=sys.stderr,
        )
        return 1
