...
Usage:
  python -m evals.experiments.probe_scale_1000            # 1000 chunks, no rerank
  python -m evals.experiments.probe_scale_1000 --target 10000
  python -m evals.experiments.probe_scale_1000 --target 10000 --rerank
  python -m evals.experiments.probe_scale_1000 --target 200 --template-only  # smoke
"""
...
def _run_eval(retriever: str, report_path: Path, extra: list[str] | None = None) -> dict:
    ...
    The subprocess runs with cwd=repo root so ``python -m evals.run_eval``
...
async def main(target: int, template_only: bool, with_rerank: bool) -> None:
    ...
    if seed_count == 0:
        logger.error("No seed chunks! Run `python -m evals.seed` first.")
        return
