...
The retrieval eval's labeled set (``evals/ground_truth.py``, 30
queries) is hand-authored and static.  This script is the cold-start
expansion path: given the seed corpus (``evals/seed_memories.jsonl``,
...
Output is a candidate file (default ``evals/experiments/query_candidates.jsonl``)
...
Usage::
    ...
    python -m evals.experiments.generate_queries --dry-run

    # Generate for the first 3 memories (3 LLM calls), merge into the file
    python -m evals.experiments.generate_queries --limit 3

    # Narrow to specific seeds
    python -m evals.experiments.generate_queries --seed seed-001,seed-007

    # Re-generate one seed, replacing its old candidates
    python -m evals.experiments.generate_queries --seed seed-001 --force
"""
...
from evals.ground_truth import DIFFICULTIES
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m evals.experiments.generate_queries",
