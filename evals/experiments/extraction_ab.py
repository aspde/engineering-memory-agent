...
(``evals/reports/llm-eval-baseline.json`` — zero-shot + json, 2026-08-09,
same provider):
...
Usage:
    python -m evals.experiments.extraction_ab --report-md evals/reports/archive/extraction_ab_report.md
"""
...
from evals.llm_ground_truth import load_extraction_items
from evals.llm_runner import run_extraction

# ``parent`` × 3 climbs from experiments/ to the repo root.
_BASELINE_PATH = Path(__file__).resolve().parent.parent.parent / "evals" / "reports" / "llm-eval-baseline.json"
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m evals.experiments.extraction_ab",
