...
The eval's LLM-as-judge (:func:`evals.llm_judge.judge_answer`) grades
...
human grader can be quantified (see ``evals.experiments.judge_calibration``).

Design rules:

- Every sample reuses the ``query`` / ``context`` / ``required_facts`` of one
  :data:`evals.llm_ground_truth.ANSWER_ITEMS` item verbatim (drift-free
...
from evals.llm_ground_truth import ANSWER_ITEMS, AnswerItem
