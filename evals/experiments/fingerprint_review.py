...
Usage::
    ...
    python -m evals.experiments.fingerprint_review

    # Include candidate rows regardless of their review status
    python -m evals.experiments.fingerprint_review --all

    # Apply mode: approved + chosen rows → ground_truth_additions.jsonl
    python -m evals.experiments.fingerprint_review --apply

    # JSONL mode: render the same additions as paste-ready JSONL rows — one
    # labeled query per line, appendable to evals/data/ground_truth.jsonl
    python -m evals.experiments.fingerprint_review --code
    python -m evals.experiments.fingerprint_review --code --out /tmp/entries.txt
...
from evals.dataset import SeedMemory, load_seed_memories
...
# ── JSONL rendering (--code) ─────────────────────────────────────
# ``--apply`` writes machine-readable JSONL; ``--code`` renders the same
# additions as paste-ready JSONL lines — one labeled query per line, exactly
# the shape ``evals.data.ground_truth.jsonl`` loads — so the manual merge
# step is appending the block and running ``--validate-only``.


def render_jsonl_lines(additions: Sequence[dict[str, Any]]) -> str:
    """Render *additions* as JSONL lines for ``data/ground_truth.jsonl``.

    Each addition is one JSON object per line (the retrieval ground-truth
    loader's shape: id / query / seed_ids / relevant_fingerprints / category /
    difficulty / notes).  The block appends directly to
    ``evals/data/ground_truth.jsonl``; validate with
    ``python -m evals.run_eval --validate-only``.
    """
    return "".join(
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m evals.experiments.fingerprint_review",
    ...
    p.add_argument(
        ...
        help="Render approved+chosen rows as JSONL lines for "
        "evals/data/ground_truth.jsonl (printed to stdout unless --out "
        "is set). Mutually exclusive with --apply.",
    )
    p.add_argument(
        "--out", default=None,
        help="Where to write the --code JSONL lines. Default: stdout.",
    )
    return p
...
def _review(args: argparse.Namespace, seeds: Sequence[SeedMemory]) -> int:
    ...
    if not candidates:
        print(
            ...
            "evals.experiments.generate_queries first.",
            file=sys.stderr,
        )
        return 1
...
def _code(args: argparse.Namespace, seeds: Sequence[SeedMemory]) -> int:
    ...
    if not additions:
        ...
        return 1
    snippet = render_jsonl_lines(additions)
    if args.out:
        Path(args.out).write_text(snippet, encoding="utf-8")
        print(f"✓ wrote JSONL additions → {args.out}", file=sys.stderr)
    else:
        print(snippet)
    return 0
