...
Examples:
    ...
    python -m evals.run_eval --retriever memory --compare

    # Chunk-table path (needs `python -m evals.seed` first)
    python -m evals.run_eval --retriever chunk --report-md report.md

    # Filter to one category
    python -m evals.run_eval --retriever memory --category 技术决策

    # Save machine-readable JSON for CI gating / trend tracking
    python -m evals.run_eval --retriever memory --report-json report.json

    # Regression gate: fail (exit 2) if overall metrics fall below thresholds
    python -m evals.run_eval --retriever memory --min-recall@5 0.95 --min-mrr 0.90
"""
...
from evals.dataset import rerank_tag, validate_dataset
from evals.report import (
    summarize,
    write_json,
    write_markdown,
)
from evals.runner import EvalConfig, compare_eval, run_eval
...
def _build_thresholds(args: argparse.Namespace) -> dict[str, float]:
    ...
    :func:`~evals.thresholds.check_thresholds`.
    """
