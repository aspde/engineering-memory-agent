"""Tests for the eval runners — orchestration + aggregation.

Covers ``evals/llm_runner.py`` (tool selection / extraction / answer / e2e
suites) and ``evals/task_runner.py`` (the task-level suite).  Both use fake
executors (no LLM / no DB) so the tests are fast and deterministic.  The
shared contract under test:
    ...
    - judge degradation is recorded separately in judge_errors: the extraction
      ...
      the answer / e2e / task suites zero the judge-owned metric keys (rather
      than re-feeding the deterministic channel into the same gate keys)
    - judge mode selection controls which metric keys appear

(The suites live in separate runner modules because their executors and
ground-truth shapes differ; the contract tests sit side by side here so a
contract change is made — and reviewed — once.)
"""
...
import evals.llm_runner as runner_mod
from evals.llm_executors import E2EOutcome
...
from evals.llm_runner import (
    run_answer,
    run_e2e,
    run_extraction,
    run_tool_selection,
)
from evals.task_executors import TaskOutcome
from evals.task_ground_truth import TaskItem
from evals.task_runner import run_tasks
...
class TestRunTasks:
    ...
    async def test_judge_channel_overrides(self, monkeypatch) -> None:
        import evals.task_runner as runner_mod
