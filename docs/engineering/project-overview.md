...
`R = max(e^(-t/S), 0.1)`，`S = 1+(recall+1)×12`，检索排序用"相似度 × decay_factor"。频繁被问到的记忆衰减更慢，长期没人问的自然沉底——记忆库能自我演化，不用人工清理。系数经 A/B 实测调校：原系数让旧记忆衰减太快、把相关旧知识整体埋掉，调慢曲线并加保留下限后，评估集召回从 0.37 回升到 0.67。
