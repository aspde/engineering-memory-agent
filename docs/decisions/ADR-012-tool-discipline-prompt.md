# ADR-012: 工具纪律 prompt（agent.system v6→v7）与检索工具描述边界
...
- **task-008 已知取舍（2026-09-05 已收口）**：该任务（问 EMA 自身功能）改动后三次复测连续零调用直接作答（改动前调 3 次 search）。已验证两个事实当前均可检索到——不是检索不到，而是模型判断"问我自己的功能不需要查库"；答案 groundedness 保持 1.000 无捏造。这是停手纪律在 agent 自身功能类问题上的副作用，严格轨迹分归零。收口：agent.system v6→v7 增加"操作本系统的问题也是团队知识、先搜长期记忆"条款 + `search_memories_tool` docstring 同向边界；task-005/008 同批收口，见 [task_eval_report.md](../../evals/reports/task_eval_report.md)。
...
2. ~~task-008 式零调用扩散到其他操作类/知识类任务~~（已收口：v7 + docstring 覆盖 agent 自身用法类问题，2026-09-05；若收口后仍复现零调用，则升级为机制层修复）；
3. 新任务类别引入后停手纪律系统性漏检。

另见配套的环境噪声分离（`outcome_class` / `completed_clean`，2026-09-05）：后续复测的
completed 结论以 `completed_clean` 为准——污染轮不再需要人工写 caveat。

## 后果

- `backend/service/prompts.py` agent.system v6（2026-08-24），v7 收口后为最终版本（见「代价与保留」task-008 节）；快照已重生成（`tests/unit/_prompt_snapshot.json`）。
...
- 复测数据见 `evals/reports/task_eval_report.md`「工具纪律 prompt（agent.system v6）复测」节（v7 收口见同文件 2026-09-05 节）与 `evals/reports/task_eval_{pre,post}_discipline_run*.md`。
