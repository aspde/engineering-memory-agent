...
EMA 基础功能已就绪：ReAct Agent 循环、三阶段记忆提取、四级相似度去重、艾宾浩斯衰减、HITL 审批。项目进入进阶扩展阶段，需要在多个可能方向中做出优先级决策。
...
2. **零新依赖**：实体归一化用 LLM + pgvector 向量相似度，一度关系查询用纯 SQL，不需要引入图数据库（延续 [ADR-005](./ADR-005-no-neo4j.md) 决策）。
...
| 图数据库（Neo4j 等） | **不做** | 延续 ADR-005；一度关系纯 SQL 即可；多跳遍历可先用递归 CTE |
