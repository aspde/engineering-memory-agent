...
| 术语 | 定义 | 当前状态 |
|------|------|---------|
| **Memory（记忆）** | 从原始内容中提取的结构化长期知识单元。包含摘要、实体、关系、向量、召回统计。 | ✅ 已实现（`memories` 表） |
...
| **Recall（召回统计）** | 记忆被检索的次数与最近时间（`recall_count`/`recalled_at`），作访问历史元数据。原艾宾浩斯衰减加权已因 A/B 掉 recall 移除（见 [decision-faq 第 7 节](../engineering/decision-faq.md)）。 | ✅ 已实现 |
...
- **记忆系统**: 参见 [`memory-system.md`](../memory-system.md) — 提取、去重、召回统计
