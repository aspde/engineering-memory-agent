...
新增三层主动能力：**定时巡检**（固定频率的全面扫描）、**事件驱动响应**（外部事件触发即时分析）、**主动通知**（将发现推送到飞书/Web UI）。每层都是同一个 Agent Graph 配上不同的触发方式和 System Prompt——架构不变，只换装配方式。
...
2. As a team, I want the daily brief to include pattern matches — "PingCode #EMA-58 'API 响应变慢' has symptoms highly similar (0.89) to the March connection pool outage" — so that recurring issues are flagged early.
...
4. As a team, I want the daily brief to surface newly discovered entities — "Kafka appeared in 12 飞书 messages this week but has zero memories" — so that emerging technologies are noticed before they become unmanaged dependencies.
...
8. As a team, I want EMA to react to a PingCode work item being marked "resolved" — EMA should search for related historical issues with the same root cause and flag if this looks like a repeat, so that we notice when we're fixing the same thing repeatedly.
...
11. As a team, I want patrol findings and event alerts pushed to a designated 飞书 group, so that we receive insights without checking EMA's Web UI.
...
### 新增 notify_feishu tool

Agent 完成巡检后，如果需要推送，调用新 tool：

```
notify_feishu(message: str, msg_type: "text" | "interactive", title: str | None)
```

这个 tool 的唯一职责是将结果格式化并通过飞书 bot Webhook 发送。前端巡检结果展示走现有 API，不经过飞书。
...
| Tool | 职责 |
|------|------|
| `notify_feishu` | 将巡检结果推送到飞书。Web UI 展示不走此 tool |
...
- `notify_feishu` tool 只测试格式化逻辑和参数传递——Mock 飞书 webhook 调用
...
| `tests/unit/test_notify_feishu.py` | 消息格式化、参数传递 | test_agent_tools.py |
| `tests/api/test_patrol_routes.py` | 手动触发、日志查询、finding dismiss | test_memory_routes.py |
| `tests/unit/test_agent_tools.py` | notify_feishu tool 返回格式 | 已有测试扩展 |
...
- 通知渠道扩展（邮件、短信、PagerDuty）——首批只做飞书 + Web UI
