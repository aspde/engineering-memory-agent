...
- `EVENT_ANALYSIS_*` — 事件驱动分析（Phase 3 事件响应，**默认关闭**，外层仍受 `CONNECTORS_ENABLED` 门控）：`EVENT_ANALYSIS_ENABLED=true` 后， opted-in 连接器的事件投递在终态后由分析 Agent（只读检索工具面，不写记忆）检索相似历史事件并输出结构化结论——结论落 `webhook_logs.analysis` 列，severity 达到阈值时推送飞书卡片（需 `FEISHU_WEBHOOK_URL`）。当前仅 CI 失败分析接线；机制按 connector 泛化（能力属性声明），故开关不带 CI 前缀。同一实体（CI: job_name）在冷却窗口内只分析一次（防 CI 风暴逐条烧 Agent）。其余项：`EVENT_ANALYSIS_COOLDOWN_SECONDS`（同实体冷却窗口，默认 3600，对应 spec story 10 的"一小时风暴只告警一次"语义）/ `EVENT_ANALYSIS_TIMEOUT_SECONDS`（单次分析超时，默认 120）/ `EVENT_ANALYSIS_MAX_CONCURRENCY`（并发上限，独立于提取槽位，默认 2）/ `EVENT_ANALYSIS_NOTIFY_SEVERITY`（推送阈值 info|warning|critical，默认 warning）/ `EVENT_ANALYSIS_NOTIFY_ENABLED`（通知总开关，默认 true）。分析的 trace_id 复用投递 id（`webhook:{delivery_id}`），`/api/usage/trace/{id}` 可回放摄入 + 分析全链路。实现见 `backend/runner/event_analysis.py`。另有自动复盘内层开关（Phase 4）：`EVENT_POSTMORTEM_ENABLED`（默认 false）+ `EVENT_POSTMORTEM_SEVERITY`（默认 critical）——分析判定已知问题重演且达到阈值时自动运行故障复盘场景（落 `scenario_runs` 表、推飞书卡片，同实体 6h 冷却），外层仍需 `SCENARIOS_ENABLED=true`。
- `SCENARIOS_ENABLED` — 垂直场景模块总开关（**默认 false**，ADR-011）。false 时 `/api/scenarios*` 路由不挂载（404）、事件触发路径也不运行场景（`maybe_trigger_postmortem` 同受此门控——`EVENT_POSTMORTEM_ENABLED` 是内层开关，`SCENARIOS_ENABLED` 是外层总闸）；场景有真实输入时置 `true`。其余项：`SCENARIO_TIMEOUT_SECONDS`（单场景超时，默认 600，超时返回 504）/ `SCENARIO_MAX_STEPS`（场景 ReAct 步数上限，默认 12——compose 链需多轮检索，交互预算 5 会中途收束）/ `SCENARIO_MAX_TOKENS`（场景最终合成输出上限，默认 8000）/ `MAX_SCENARIO_CONCURRENCY`（并发场景上限，默认 2，超限返回 503）
...
| 对话 checkpoint（`AsyncPostgresSaver` 连接池） | `backend/runner/agent_service.py` | 唯一已持久化的状态；`checkpoints` 表在 PG 中，多副本下对话历史仍一致，但同一 `thread_id` 的并发续写由哪副本处理不可控 |
