...
Usage::
    ...
    python -m tests.eval.experiments.fingerprint_review

    # Include candidate rows regardless of their review status
    python -m tests.eval.experiments.fingerprint_review --all

    # Apply mode: approved + chosen rows → ground_truth_additions.jsonl
    python -m tests.eval.experiments.fingerprint_review --apply

    # JSONL mode: render the same additions as paste-ready JSONL rows — one
    # labeled query per line, appendable to tests/eval/data/ground_truth.jsonl
    python -m tests.eval.experiments.fingerprint_review --code
    python -m tests.eval.experiments.fingerprint_review --code --out /tmp/entries.txt
...
# the shape ``tests.eval.data.ground_truth.jsonl`` loads — so the manual merge
...
def render_jsonl_lines(additions: Sequence[dict[str, Any]]) -> str:
    ...
    ``tests/eval/data/ground_truth.jsonl``; validate with
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m tests.eval.experiments.fingerprint_review",
    ...
    p.add_argument(
        ...
        "tests/eval/data/ground_truth.jsonl (printed to stdout unless --out "
...
def _review(args: argparse.Namespace, seeds: Sequence[SeedMemory]) -> int:
    ...
    if not candidates:
        print(
            ...
            "tests.eval.experiments.generate_queries first.",
            file=sys.stderr,
        )
        return 1
