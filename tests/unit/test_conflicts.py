"""Unit tests for backend/service/conflicts.py — the pending-conflict queue.

Two conflict kinds flow through the shared HITL arbitration pipeline:

- **Ingestion conflicts** — ``write_memory`` detects a near-duplicate and
  queues the new content for human review.
- **Patrol contradictions** — the patrol flags two *already-stored* memories
  (A, B) as contradictory and queues them as a pair.

DB access is mocked at the session-factory boundary; the LLM and embedding
providers are mocked so these stay hermetic.
"""
...
# ---------------------------------------------------------------------------
# Shared arbitration dispatch (both kinds resolve through one endpoint)
# ---------------------------------------------------------------------------


class TestResolvePendingConflictDispatch:
    @pytest.mark.asyncio
    async def test_ingestion_row_delegates_and_marks_resolved(self) -> None:
        """An ingestion row reaches resolve_conflict with the parsed deferred
        payload and peer_id=None, then the row is marked resolved."""
        from backend.service.conflicts import resolve_pending_conflict

        deferred = {"content_hash": "abc123"}
        conflict_row = MagicMock()
        conflict_row.existing_id = "11111111-1111-1111-1111-111111111111"
        conflict_row.status = "pending"
        conflict_row.deferred = json.dumps(deferred)  # asyncpg returns jsonb as str
        ...
        # Delegated to the shared pipeline with the parsed stored payload.
    ...
    @pytest.mark.asyncio
    async def test_patrol_row_forwards_peer_id(self) -> None:
...
# ---------------------------------------------------------------------------
# resolve_conflict pipeline (backend.service.memory.resolve_conflict)
# ---------------------------------------------------------------------------
